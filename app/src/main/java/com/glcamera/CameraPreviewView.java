...
public class CameraPreviewView extends GLSurfaceView implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener, View.OnClickListener {
    private final String vss =
            "attribute vec2 vPosition;\n" +
                    ...
                    "}";
//    private static final String vss =           竖屏情况用这个
...
//                "}\n";
    ...
    private static String LOG_TAG = CameraPreviewView.class.getName();
    ...
    private float mPreviewRatio = 640 / 480.0f;//1.581f;//
    private float mPreviewScale = 0.9f;//缩放比例
    ...
    private void initView() {
        ...
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            mPreviewRatio = (float) size.height / size.width;
        }
    ...
    public class Square {
        ...
        //矩形顶点坐标
        private float squareCoords[] = { 0.24f,  0.62f ,   // top left  符合数学坐标系 中心点在屏幕中间  最大值为1 最小值为-1
                0.24f, -0.48f ,   // bottom left
                0.81f, -0.48f ,   // bottom right
                0.81f,  0.62f }; // top right
...
//            GLES20.glUseProgram(mProgram);
        ...
        // 设置颜色和透明度（r,g,b,a）
        private final float color[] = {0.0f, 1.0f, 0f, 1.0f};//绿色不透明
...
//        int th = GLES20.glGetUniformLocation(hProgram, "uMVPMatrix");
//        checkLocation(th, "uMVPMatrix");
//
...
//        checkGlError("glUniformMatrix4fv");
        ...
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);//width:680 h:430
        {
            double aspectHeight = mWindowHeight * mPreviewScale * 0.85f;//430
            double aspectWidth = mWindowHeight / mPreviewRatio  * mPreviewScale;
            if (mWindowWidth > aspectWidth) {
                aspectWidth = mWindowWidth  * mPreviewScale ;//680
                aspectHeight = mWindowWidth * mPreviewRatio  * mPreviewScale * 0.85f;
            }
